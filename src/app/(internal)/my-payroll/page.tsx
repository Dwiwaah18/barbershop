...
import MyPayrollClient from './my-payroll-client';
...
export default async function MyPayrollPage() {
  ...
  if (!current) {
    return (
      <div>
        <h1 className="text-3xl font-bold mb-8">Payroll</h1>
  ...
  return (
    <div>
      ...
      <p className="text-gray-400 mb-8">
        Slip gaji kamu berdasarkan skema komisi cabang dan rentang tanggal yang dipilih.
      </p>
      {branches.length === 0 ? (
        <div className="glass-panel p-6 rounded-2xl text-gray-400">
        ...
        <MyPayrollClient branches={branches} userId={current.userId} fullName={current.profile.full_name ?? 'Tanpa nama'} />
      )}
    </div>
  );
}
