...
export default async function TenantBranchesPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const supabase = await createClient();
  const tenant = await getTenantBySlug(supabase, slug);
  if (!tenant) notFound();

  const { data } = await supabase.from('branches').select('*').eq('tenant_id', tenant.id).order('name');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {branches.map((branch) => (
            <div key={branch.id} className="glass-panel p-6 rounded-2xl flex flex-col group hover:border-primary/50 transition-colors">
              ...
              <Link
                href={`/toko/${slug}/booking?branch=${branch.id}`}
                className="mt-auto w-full inline-flex items-center justify-center px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl transition-colors font-medium border border-[var(--border)]"
              >
