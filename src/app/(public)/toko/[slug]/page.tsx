...
const dashboardByRole: Record<string, { label: string; href: string }> = {
  owner: { label: 'Owner Dashboard', href: '/dashboard' },
  superadmin: { label: 'Owner Dashboard', href: '/dashboard' },
  cashier: { label: 'Kasir Dashboard', href: '/queue' },
  barber: { label: 'Kapster Dashboard', href: '/attendance' },
};
...
export default async function TenantHome({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const staffDashboard = current ? dashboardByRole[current.profile.role] : undefined;

  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 sm:px-6 lg:px-8">
      {/* Hero Section */}
      <div className="text-center max-w-4xl mx-auto mb-16">
        <p className="text-primary font-semibold mb-3">{tenant.name}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link
            href={`/toko/${tenant.slug}/booking`}
            className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold rounded-full text-primary-foreground bg-primary hover:bg-amber-700 transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(217,119,6,0.4)]"
          >
            Book Appointment <ArrowRight className="ml-2 h-5 w-5" />
          </Link>
          {staffDashboard && (
            <Link href={staffDashboard.href} className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold rounded-full text-white bg-white/10 hover:bg-white/20 backdrop-blur-md transition-all border border-white/10">
